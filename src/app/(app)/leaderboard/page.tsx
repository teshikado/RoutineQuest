...
const MEDAL_COLORS = ["#FACC15", "#C0C0CE", "#D4915A"];
  ...
  if (memberships.length === 0) {
    return (
      <div className="space-y-6">
        <h1 className="text-2xl font-extrabold text-[#F8F7FC]">Rangliste</h1>
        <EmptyState
          icon="Trophy"
    ...
    <div className="space-y-6">
      ...
      <RevealGroup className="grid grid-cols-1 sm:grid-cols-3 gap-4" stagger={0.06}>
        <Card className="flex items-center gap-3">
          <Trophy className="h-8 w-8 text-[#FACC15] shrink-0" />
          <div>
            <div className="text-lg font-extrabold text-[#F8F7FC] tabular-nums">{data.groupTasksCompleted}</div>
            <div className="text-xs text-[#C8C5D2]">Aufgaben diese Woche als Gruppe</div>
          </div>
        </Card>
        {data.me && (
          <Card className="flex items-center gap-3">
            <div className="text-lg font-extrabold text-[#34D399] tabular-nums">
              {data.me.improvementPct >= 0 ? "+" : ""}
              {data.me.improvementPct}%
            </div>
            <div className="text-xs text-[#C8C5D2]">Deine Verbesserung ggü. letzter Woche</div>
          </Card>
        )}
        {data.xpToNextPlace !== null && data.xpToNextPlace > 0 && (
          <Card className="flex items-center gap-3">
            <div className="text-lg font-extrabold text-[#A855F7] tabular-nums">{data.xpToNextPlace} XP</div>
            <div className="text-xs text-[#C8C5D2]">Bis zum nächsten Platz</div>
          </Card>
        )}
      </RevealGroup>
      ...
      <Reveal delay={0.08}>
        <Card>
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="font-bold text-[#F8F7FC]">{selectedGroup.name}</span>
            <span className="text-xs text-[#8D8998]">· Woche {data.week}</span>
          </div>
            ...
            {data.members.map((m) => (
              <div
                ...
                className={`flex items-center gap-3 rounded-xl p-3 transition-all duration-200 hover:-translate-y-0.5 ${
                  m.userId === userId
                    ? "bg-[#171720] shadow-[inset_0_0_0_1px_rgba(168,85,247,0.4)]"
                    : "bg-[#171720] hover:shadow-[var(--shadow-xs)]"
                }`}
              >
                <div className="w-7 flex justify-center">
                  {m.position <= 3 ? (
                    ...
                    <span className="text-sm font-bold text-[#8D8998] tabular-nums">{m.position}</span>
                  )}
                </div>
                <div
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-semibold text-[#F8F7FC] truncate">{m.username}</div>
                  <div className="text-xs text-[#C8C5D2] flex items-center gap-1.5">
                ...
                <div className="text-right shrink-0">
                  <div className="font-extrabold text-[#F8F7FC] tabular-nums">{m.weeklyXp} XP</div>
                  <div className="text-[10px] text-[#8D8998]">🔥 {m.currentStreak}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <p className="text-xs text-[#8D8998] text-center">
